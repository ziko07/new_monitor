...
test("medicare Rapidfire Prefill Lead Mobile Test", async ({ browser }) => {
  ...
  const response = await page.goto(process.env.ENVIRONMENT_URL || 'https://smartfinancial.com/get-fast-quote-medicare.html#/quote-ready?session_id=bb96a5e9-d969-418b-b2a5-43471c3a8a26');
  await advantage_step(page)
...
test('medicare Rapidfire Prefill Lead Desktop Test ', async ({ page }) => {
...
async function advantage_step(page) {
  const advantage_div = page.locator('.quote-ready-page');
  await expect(advantage_div).toBeVisible();
  // Randomly select Yes or No
  const randomSelection = Math.random() < 0.5 ? 'yes' : 'no';
   

  if (randomSelection === 'yes') {
    await page.click('label[for="compare-rates-yes"]');  // Select Yes
  } else {
    await page.click('label[for="compare-rates-no"]');
  }

  const continue_btn = await advantage_div.locator(".btn.btn-custom.btn-block");
...
async function phone_step(page) {
  ...
  const phone_div = page.locator('.quote-summary');
  await expect(phone_div).toBeVisible();

  // Click on the "Select All" checkbox
   
  await page.click('label[for="compare-company-all"]');
